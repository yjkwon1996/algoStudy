# 웜홀. 때는 2020년, 백준이는 월드나라의 한 국민이다. 월드나라에는 N개의 지점이 있고 N개의 지점 사이에는 M개의 도로와 W개의 웜홀이 있다. (단 도로는 방향이 없으며 웜홀은 방향이 있다.)
...
def bellmanFord() : # 벨만포드알고리즘
    dist = [sys.maxsize for _ in range(N+1)]
    dist[1] = 0

    for i in range(N) :
        for start, goal, time in arr :
            if dist[goal] > dist[start] + time :
                dist[goal] = dist[start] + time
