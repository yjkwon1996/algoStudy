# 24. 도현이는 Counter Terror Unit (CTU)에서 일하는 특수요원이다. 도현이는 모든 사건을 정확하게 24시간이 되는 순간 해결하는 것으로 유명하다.
...
nowH, nowM, nowS = map(int, input().rstrip().split(":"))
startH, startM, startS = map(int, input().rstrip().split(":"))

now = (nowH * 60 * 60) + (nowM * 60) + nowS
start = (startH * 60 * 60) + (startM * 60) + startS

if start < now :
    t = (60*60*24) + (start - now)
else :
    t = start - now
