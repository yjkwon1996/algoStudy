# 임시 반장 정하기. 오민식 선생님은 올해 형택초등학교 6학년 1반 담임을 맡게 되었다. 오민식 선생님은 우선 임시로 반장을 정하고 학생들이 서로 친숙해진 후에 정식으로 선거를 통해 반장을 선출하려고 한다.
...
# 그러므로 이 학급에서 4번 학생과 한번이라도 같은 반이었던 사람은 2번 학생, 3번 학생과 5번 학생으로 모두 3명이다. 이 예에서 4번 학생이 전체 학생 중에서 같은 반이었던 학생 수가 제일 많으므로 임시 반장이 된다.
#
...
for k in range(5) :
    for i in range(num) :
        for j in range(i+1, num) :
            if arr[i][k] == arr[j][k] :
                result[i][j] = 1 # i와 j가 같은반을 했음
                result[j][i] = 1

cnt = []
# 각각으로 1인 경우를 체크
for v in result :
    cnt.append(v.count(1))

print(cnt.index(max(cnt))+1) # 가장 많은 사람과 같은 반을 한 사람(idx+1)
