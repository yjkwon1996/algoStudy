# 이중 우선순위 큐. 이중 우선순위 큐(dual priority queue)는 전형적인 우선순위 큐처럼 데이터를 삽입, 삭제할 수 있는 자료 구조이다.
...
for _ in range(T) :
    k = int(input().rstrip()) # Q에 적용할 연산의 개수
    maxHeap = []
    minHeap = []
    visited = [False for _ in range(1000001)]

    for i in range(k) :
        chr, n = input().rstrip().split()

        if chr == 'I' :
            heapq.heappush(minHeap, (int(n), i))
            heapq.heappush(maxHeap, (int(n)*-1, i))
            visited[i] = True
