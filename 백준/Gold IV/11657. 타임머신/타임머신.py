# 타임머신. N개의 도시가 있다. 그리고 한 도시에서 출발하여 다른 도시에 도착하는 버스가 M개 있다.
...
def bellmanFord(x) :
    ...
    for i in range(1, N+1) :
        for j in range(M) :
            now, next, cost = bus[j][0], bus[j][1], bus[j][2]

            # 간선이 더 짧은 방향으로 갱신
            if dist[now] != sys.maxsize and dist[next] > dist[now] + cost :
                dist[next] = dist[now] + cost

                if i == N : # N번째에도 값이 갱신됐다면 음수 순환. 무한히 과거로 가는 경우
                    return False
...
value = bellmanFord(1) # 1번도시에서 출발

if not value :
    print(-1)
else :
    for i in range(2, N+1) :
        if dist[i] == sys.maxsize : # 목적지에 도달할 수 없음
            print(-1)
        else :
            print(dist[i])
