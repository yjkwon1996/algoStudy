...
for test_case in range(1, T + 1):
    N = int(input())
    matrix = [[0] * N for _ in range(N)] # NxN 배열
    
    for i in range(N) :
        for j in range(i+1) :
            if j == 0 or i == j : # 첫째 라인과 마지막 라인은 항상 1
                matrix[i][j] = 1
            else : # [i][j]는 왼쪽위[i-1][j-1]과 오른쪽위[i-1][j]를 더한 값
                matrix[i][j] = matrix[i-1][j-1] + matrix[i-1][j]
            
            
    print("#{}".format(test_case))
    
    for i in matrix : # 0 빼고 출력하면 파스칼의 삼각형
        answer = [x for x in i if x != 0]
        print(*answer)
